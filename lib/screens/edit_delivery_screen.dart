...
import 'add_delivery_screen.dart'; // To import ProductItemEntry
...
class _EditDeliveryScreenState extends State<EditDeliveryScreen> {
  ...
  void initState() {
    ...
    // Ensure dropdown has a stable initial value even before API results arrive.
    ...
    if (_productEntries.isEmpty) {
      _productEntries.add(ProductItemEntry());
    }
  ...
  Future<void> _fetchCustomers() async {
    ...
    try {
      ...
      // De-dupe and keep the currently selected customer even if not in API data
      final set = <String>{
        ...names,
  ...
  void dispose() {
    ...
    // Dispose per-entry controllers to prevent memory leaks
    ...
    try {
      ...
      final updatedDelivery = Delivery(
        ...
        invoice: DeliveryInvoice(
          ...
          customFields: _customFieldsControllers.map((field) {
            return {
      ...
      final success =
          await _apiService.updateDelivery(widget.delivery.id, updatedDelivery);
      ...
      if (success && mounted) {
        ...
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(
              content: Text('Delivery updated! Preparing updated invoice...')),
        );

        // Generate PDF and share — so the customer gets the updated invoice
        // immediately. Falls back to print preview if share is unavailable.
        try {
          final invoice =
              InvoiceModel.fromDelivery(updatedDelivery, 'updated');
          final pdfBytes = await _pdfService.generateInvoice(invoice);

          final dir = await getTemporaryDirectory();
          final file = File(
              '${dir.path}/Updated_Invoice_${widget.delivery.id}.pdf');
          ...
          if (mounted) {
            await Share.shareXFiles(
              ...
              subject:
                  'Updated Invoice — ${updatedDelivery.customerName}',
              text:
                  'Please find the updated invoice for delivery ${widget.delivery.id}.',
            );
          }
        } catch (shareError) {
          // Share failed (e.g. no share targets on emulator) — fall back to
          // in-app PDF preview so the user still sees the updated invoice.
          try {
            final invoice =
                InvoiceModel.fromDelivery(updatedDelivery, 'updated');
            await Printing.layoutPdf(
              onLayout: (format) async =>
                  _pdfService.generateInvoice(invoice),
              name: 'Updated_Invoice_${widget.delivery.id}.pdf',
            );
          } catch (_) {
            // Non-critical — silently ignore preview failure
          }
        }
            ...
            child: SingleChildScrollView(
              ...
              child: Column(
                ...
                children: [
                  ...
                  Text(
                    'Modify delivery details and pricing.',
                            ...
                            children: [
                              ...
                              Expanded(
                                child: Text(
                                  _isLoadingCustomers
                                      ? 'Loading customers...'
                                      : 'Could not refresh customers. Using existing selection.',
                                  style: GoogleFonts.inter(
                                    fontSize: 12,
                        ...
                        child: Column(
                          ...
                          children: [
                            Row(
                              mainAxisAlignment: MainAxisAlignment.spaceBetween,
                              children: [
                                Text('Item #${index + 1}', style: GoogleFonts.inter(fontWeight: FontWeight.bold, color: AppColors.primaryGreen)),
                            ...
                            const SizedBox(height: 12),
                            // Bug fix: value: bound to model so dropdown survives setState rebuilds
                            DropdownButtonFormField<String>(
                              ...
                              onChanged: (v) => setState(() => item.productName = v),
                              ...
                              children: [
                                Expanded(
                                  ...
                                  child: TextFormField(
                                    ...
                                    decoration: _inputDecoration('Price / ${item.pricingType == 'Per KG' ? 'KG' : 'Bag'}', Icons.payments_outlined),
                            ...
                            Container(
                              ...
                              child: Row(
                                mainAxisAlignment: MainAxisAlignment.spaceBetween,
                                children: [
                                  Text('Subtotal:', style: GoogleFonts.inter(fontSize: 12, fontWeight: FontWeight.w600, color: AppColors.textSecondary)),
                                  Text('₹ ${item.subtotal.toStringAsFixed(2)}', style: GoogleFonts.inter(fontWeight: FontWeight.bold, color: AppColors.primaryGreen)),
                                ],
                              ),
                            ),
                          ],
                    ...
                    child: Column(
                      children: [
                        Row(
                          mainAxisAlignment: MainAxisAlignment.spaceBetween,
                          children: [
                            Text('GRAND TOTAL', style: GoogleFonts.inter(color: Colors.white.withValues(alpha: 0.8), fontWeight: FontWeight.bold, letterSpacing: 1)),
                            Text('₹ ${_calculateGrandTotal.toStringAsFixed(2)}', style: GoogleFonts.inter(color: Colors.white, fontSize: 24, fontWeight: FontWeight.bold)),
                          ],
                        ),
                        const SizedBox(height: 16),
                        PrimaryButton(
                          text: 'Update Delivery',
                          onPressed: _handleUpdateDelivery,
                          showArrow: false,
                        ),
                      ],
                    ),
  ...
  Widget _buildPricingTypeToggle(ProductItemEntry item, String type) {
    bool isSelected = item.pricingType == type;
    return InkWell(
